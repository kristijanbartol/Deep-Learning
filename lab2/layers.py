...
from lab2.im2col_cython import col2im_cython, im2col_cython
...
def softmax(x):
    ...
    return [math.exp(xi) / denom for xi in x]
...
class SoftmaxCrossEntropyWithLogits():
    ...
    def forward(self, x, y):
        ...
        """
        return - 1 / x.shape[0] * np.sum(np.multiply(y, np.log(np.apply_along_axis(softmax, 1, x))))
    ...
    def backward_inputs(self, x, y):
        ...
        """
        return 1 / x.shape[0] * (np.apply_along_axis(softmax, 1, x) - y)
